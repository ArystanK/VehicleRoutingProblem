...
fun travelingSalesmanProblemLinearProgramming(
    ...
    if (distanceMatrix.size == 1)
        return arrayOf(booleanArrayOf(true))
    ...
    fun MPSolver.makeSubTourElimination(x: Array<Array<MPVariable>>) {
        val u = Array(distanceMatrix.size) {
            makeIntVar(2.0, distanceMatrix.size.toDouble(), "u[$it]")
        }
        for (i in 1 until u.size) {
            for (j in 1 until u.size) {
                if (i == j) continue
                val c = makeConstraint(
                    Double.NEGATIVE_INFINITY,
                    distanceMatrix.size.toDouble() - 2
                )
                c.setCoefficient(u[i], 1.0)
                c.setCoefficient(u[j], -1.0)
                c.setCoefficient(
                    x[i][j],
                    distanceMatrix.size.toDouble() - 1
                )
            }
        }
    }

    fun Array<Array<MPVariable>>.toAdjacencyMatrix()
            : Array<BooleanArray> = Array(size) { i ->
        BooleanArray(size) { j ->
            this[i][j].solutionValue() == 1.0
        }
    }

    Loader.loadNativeLibraries()
    val solver = MPSolver.createSolver("SCIP")
    val x = Array(distanceMatrix.size) { i ->
        Array(distanceMatrix.size) { j ->
            solver.makeBoolVar("x[$i, $j]")
        }
    }
